...
def translate_filter_dict(
    filter_dict: Union[dict[str, Any], list[dict[str, Any]]]
) -> str:
    ...
    if isinstance(filter_dict, list):
        return f" {filter_dict['boolean_clause']} ".join(
            process_condition(condition) for condition in filter_dict
        )
